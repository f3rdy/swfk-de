...
Die Sterntaste (*) wird anstelle des normalen Multiplikationszeichen\index{Operatoren!Multiplikation} \textsf{X} verwendet, welches du in der Schule verwendest. Die Sterntaste must du verwenden, damit der Computer weiß, ob du den Buchstaben \emph{x} oder das Multiplikationszeichen \textsf{X} meinst. Wie wäre es nun mit einer Formel, die etwas nützlicher ist?
...
Es kommt das genau das gleiche raus. Wenn du nun 2 € mehr für die Hausarbeit bekommst, kannst du einfach die `Hausarbeit' Variable auf 7 setzen. Danach drückst du die `Pfeil nach oben Taste' ($\uparrow$) und wenn die Rechnung wieder erscheint, drückst du Enter.
