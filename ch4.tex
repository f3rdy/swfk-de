...
Beim Programmieren stellt man Fragen, um je nach Antwort das Eine oder Andere zu tun. Dabei spricht man von einer \textbf{if-Bedingung}\index{if-Bedingung}. Zum Beispiel:
...
Wenn du zum Beispiel 10 Jahre alt bist, dann würde die Bedingung \code{dein\_alter == 10} zum Beispiel wahr (true) zurückgeben, wenn du aber nicht 10 bist, dann würde falsch (false) zurückkommen. Pass auf dass du die \textbf{zwei} Gleichheitszeichen (==) nicht mit dem normalen einfachen Gleichheitszeichen (=) verwechselt. Wenn du nur ein = Zeichen in einer Bedingung verwendest, dann bekommst du einen Fehler.
\par
%Assuming you set the variable \code{age} to your age, then if you are 12 years old, the condition\texorpdfstring{$\ldots$}{...}
Nehmen wir an, dass du die Variable \code{alter} auf dein Alter setzt und du 12 Jahre bist. Dann würde die Bedingung \texorpdfstring{$\ldots$}{...}
...
%\texorpdfstring{$\ldots$}{...} would again return True.  If you are 8 years old, it would return False.  If you are 10 years old, it would also return False---because the condition is checking for greater than ($>$) 10, and not greater than or equal ($>$=) to 10.
...
%\section{Do this\texorpdfstring{$\ldots$}{...} or ELSE!!!}
...
%\section{Do this\texorpdfstring{$\ldots$}{...} or do this\texorpdfstring{$\ldots$}{...} or do this\texorpdfstring{$\ldots$}{...} or ELSE!!!}
\section{Tu das\texorpdfstring{$\ldots$}{...} oder dies\texorpdfstring{$\ldots$}{...} oder jenes\texorpdfstring{$\ldots$}{...} oder!!!}
...
Im obigen Code überprüft die zweite Zeile, ob der Wert der Variable 10 entspricht. Wenn nicht springt Python zu Zeile 4 um zu überprüfen, ob der Wert der \code{alter} Variable 11 entspricht. Wenn es wieder nicht so ist, springt Python zu Zeile 6. Dort wird geschaut, ob die Variable den Wert 12 hat. In diesem Fall ist es so und deswegen kommt danach die Zeile 7 dran und der print Befehl wird ausgeführt. Wenn du den Code anschaust, erkennst du 5 Gruppen---Zeile 3, 5, 7, 9 und 11.

%\section{Combining conditions}\index{conditions!combining}
\section{Bedingungen kombinieren}\index{Bedingung!kombinieren}
...
%\section{What's the difference\texorpdfstring{$\ldots$}{...}?}\label{whatsthedifference}\index{equality}
\section{Was ist der Unterschied\texorpdfstring{$\ldots$}{...}?}\label{whatsthedifference}\index{Gleichheit}
