...
x \% 2 ist dann Null, wenn die Nummer in der Variable \code{x} durch 2 dividiert werden kann, ohne dass ein Rest übrigbleibt---wenn das keinen Sinn macht, mach dir keine Sorgen. Merke dir nur, dass du `\code{x \% 2 == 0}' verwenden kannst, um herauszufinden, ob eine Nummer gerade ist. Wenn du den Code ausführst, wirst du den 9-zackigen Stern von Abbildung~\ref{fig23} sehen.
...
Die Leinwand kannst du mit \code{schildkroete.clear()} wieder löschen.
...
Als Anfang schon ganz gut, aber noch nicht perfekt. Wenn du den Code oben anschaust, wirst du ein Muster erkennen. Wir wiederholen: \code{forward(groesse)} und \code{left(90)} vier Mal. Das ist unnötige Tipparbeit. Wir können eine for-Schleife verwenden, die uns das erspart:
...
Sollen wir unsere Funktion ein wenig verändern, damit wir entweder befüllte oder unbefüllte Quadrate zeichen können? Um das zu erreichen, verwenden wir einen zusätzlichen Parameter:
...
Die ersten zwei Zeilen überprüfen, ob der Parameter `fuellen' auf True gesetzt wurde. Wenn ja, wird \code{filling} eingeschalten. Dann wird die Schleife vier Mal durchlaufen und danach wieder der Paramter `fuellen' überprüft. Wenn der Wert True ist, dann wird \code{filling} wieder ausgeschalten. Ein gefülltes Quadrat kann also so gezeichnet werden:
...
\noindent
%Which causes our turtle to draw the image in figure~\ref{fig28}\texorpdfstring{$\ldots$}{...} \texorpdfstring{$\ldots$}{...}which, now that I think about it, looks like a weird square eye.
...
Wir können die gleiche if-Bedingung von der \code{mein\_stern} Funktion nehmen und als Parameter die Größe übergeben:
...
In Zeile 2 und 3 wird abhängig vom Parameter \code{fuellen} die Funktion \code{begin\_fill()} ausgeführt oder nicht (bei `True' ist es an, bei `False' ist es aus). Das Gegenteil machen wir bei Zeile 10 und 11. Zusätzlich übergeben wir die Größe als Parameter und verwenden sie in Zeile 5.
\par
...
\emph{In diesem Kapitel haben wir mehr über das \code{turtle} Modul gelernt und es verwendet um geometrische Formen zu zeichnen. Funktionen wurden verwendet um Code wieder zu verwenden und es einfacher zu machen verschiedene Formen zu zeichnen und zu füllen.}
